...
def model(dbt,session):
    ...
    name_vari=dbt.config.get("name")

    
    #call variable
   # if name_vari=="sunil rathod":   #"sunil rathod"
    ...
    process_date="2024-10-10"

    #create dataframe manually 

    df_report=session.create_dataframe([[process_date,as_of_Date]], schema=["as_of","process"]  )

    #udf for practice not used in my code

    @udf
    def price_formatting(pricetous:str, marketprice:str, overrideprice:str) ->str:
            price=0
            if pricetous== 'Missing Price':
                price=-1.0
            if pricetous == 'yy-yy':
                price=marketprice
            if pricetous== 'pp-pp':
    ...
    @udf
    def format_value_udf(num:float) ->str:
            if num:
    ...
    df_query_result=df_query_result.with_column("avg_sal_with_udf",format_value_udf(col("AVG_SAL")))
